...
def data_insert(info_list):
    ...
    for dic in info_list:
        item = (dic['cover'], dic['is_top'], dic['content'], dic['title'],
                dic['created'], dic['updated'], dic['is_deleted'])
        # print(item['cover'])
        val.append(item)
    sql = "INSERT INTO info_manage (cover,is_top,content,title,created,updated,is_deleted) VALUES (%s,%s,%s,%s,%s,%s,%s)"
    try:
        # 执行sql语句，批量插入
